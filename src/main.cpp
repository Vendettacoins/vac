...
unsigned int nStakeTargetSpacing = 1.5 * 60; // DIFF: 1.5-minute block spacing
int64 nChainStartTime = 1389715200;
...
const string strMessageMagic = "VACoin Signed Message:\n";
...
// VACoin: increasing Nfactor gradually
...
int64 GetProofOfWorkReward(unsigned int nBits, int nHeight)
{
    ...
    // VACoin: subsidy is cut in half every 64x multiply of PoW difficulty
    ...
    if (nHeight<1001)
        return min(nSubsidy, MAX_MINT_PROOF_OF_WORK)*1024;
    else if (nHeight>1000&&nHeight<5001)
        return min(nSubsidy, MAX_MINT_PROOF_OF_WORK)*512;
    else if (nHeight>5000&&nHeight<10001)
        return min(nSubsidy, MAX_MINT_PROOF_OF_WORK)*256;
    else if (nHeight>10000&&nHeight<15001)
        return min(nSubsidy, MAX_MINT_PROOF_OF_WORK)*128;
    else if (nHeight>15000&&nHeight<20001)
        return min(nSubsidy, MAX_MINT_PROOF_OF_WORK)*64;
    else if (nHeight>20000&&nHeight<80001)
        return min(nSubsidy, MAX_MINT_PROOF_OF_WORK)*32;
    else if (nHeight>80000&&nHeight<240001)
        return min(nSubsidy, MAX_MINT_PROOF_OF_WORK)*16;
    else if (nHeight>240000&&nHeight<480001)
        return min(nSubsidy, MAX_MINT_PROOF_OF_WORK)*8;
    else if (nHeight>480000&&nHeight<960001)
        return min(nSubsidy, MAX_MINT_PROOF_OF_WORK)*4;
    else if (nHeight>960000&&nHeight<1920001)
        return min(nSubsidy, MAX_MINT_PROOF_OF_WORK)*2;
    else
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in VACoin
    bool fStrictPayToScriptHash = true; // Always active in VACoin
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    if (vtx[0].GetValueOut() > (IsProofOfWork()? (GetProofOfWorkReward(nBits,GetLastBlockIndex(pindexBest, false)->nHeight) - vtx[0].GetMinFee() + MIN_TX_FEE) : 0))
        return DoS(50, error("CheckBlock() : coinbase reward exceeded %s > %s", 
                   FormatMoney(vtx[0].GetValueOut()).c_str(),
                   FormatMoney(IsProofOfWork()? GetProofOfWorkReward(nBits,GetLastBlockIndex(pindexBest, false)->nHeight) : 0).c_str()));
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "VACoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "And God blessed them,saying,Be frutiful,and multiply,and fill the waters in the seas,and let fowl multiply in the earth.";
        CTransaction txNew;
        ...
        block.nBits    = 0x1e0fffff;//bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 122359;

        //// debug print
        printf("block.GetHash() == %s\n", block.GetHash().ToString().c_str());
        printf("block.hashMerkleRoot == %s\n", block.hashMerkleRoot.ToString().c_str());
        assert(block.hashMerkleRoot == uint256("96d8e9ca3dff7af5befd150270d866e7e90d46a13ceca370953c39afa5670bee"));
        block.print();
...
CBlock* CreateNewBlock(CWallet* pwallet, bool fProofOfStake)
{
    ...
    int64 nFees = 0;
    {
        ...
        if (pblock->IsProofOfWork())
            pblock->vtx[0].vout[0].nValue = GetProofOfWorkReward(pblock->nBits,GetLastBlockIndex(pindexBest, false)->nHeight);
